...
public static class NiceSwapper
{
    ...
    {
        Options.SetupRoleOptions(Id, TabGroup.CrewmateRoles, CustomRoles.NiceSwapper);
        SwapMax = IntegerOptionItem.Create(Id + 3, "NiceSwapperMax", new(1, 999, 1), 3, TabGroup.CrewmateRoles, false).SetParent(Options.CustomRoleSpawnChances[CustomRoles.NiceSwapper])
    ...
    public static bool SwapMsg(PlayerControl pc, string msg, bool isUI = false)
    {
        ...
        if (!pc.Is(CustomRoles.NiceSwapper)) return false;
    ...
    private static void SwapperOnClick(byte playerId, MeetingHud __instance)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(CustomRoles.NiceSwapper) && PlayerControl.LocalPlayer.IsAlive())
        {
